...
function comparePersonaMem(result, targets) {
  ...
  if (checks.length === 0) {
    checks.push(metricResult('personamem_mcq_accuracy', aggregateMean(result, 'mcq_accuracy'), finiteScore(targets['32k']?.score, 'PersonaMem 32k target')));
  }
  return checks;
}
...
for (const check of comparison.checks ?? []) {
  ...
  assert(check.sota === true, `${check.metric} must exceed target for SOTA claim`);
}
